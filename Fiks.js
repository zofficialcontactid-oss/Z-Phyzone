...
document.getElementById('mass-left').addEventListener('input', e => { if (leftBall && !isRunning) leftBall.mass = parseFloat(e.target.value); });
document.getElementById('diameter-left').addEventListener('input', e => { if (leftBall && !isRunning) { leftBall.diameter = parseFloat(e.target.value); draw(); } });
document.getElementById('mass-right').addEventListener('input', e => { if (rightBall && !isRunning) rightBall.mass = parseFloat(e.target.value); });
document.getElementById('diameter-right').addEventListener('input', e => { if (rightBall && !isRunning) { rightBall.diameter = parseFloat(e.target.value); draw(); } });

document.getElementById('height-left').addEventListener('input', function(e) {
    document.getElementById('height-left-val').textContent = e.target.value;
    if (leftBall && !isRunning) updateBallPosition('left');
});
document.getElementById('height-right').addEventListener('input', function(e) {
    document.getElementById('height-right-val').textContent = e.target.value;
    if (rightBall && !isRunning) updateBallPosition('right');
});
...
function createBallObject(params) {
    ...
    return {
        ...
        vTerminal: 0,
        isActive: false,
        hasHitGround: false, // Flag baru pendeteksi benturan
...
function refreshBallParams(ball, side) {
    ball.mass = parseFloat(document.getElementById(`mass-${side}`).value);
    ball.diameter = parseFloat(document.getElementById(`diameter-${side}`).value);
...
function animate() {
    ...
    draw(); updateDisplays(); updateCharts();
    if (!allFinished) animationId = requestAnimationFrame(animate); else { isRunning = false; draw(); }
}

function updateBallPhysics(ball, currentTime) {
    if (ball.hasHitGround) return; // Hentikan perhitungan jika sudah menabrak
    ...
    if (height <= 0) {
        // --- LOGIKA GARIS LURUS MENUJU 0 ---
        // 1. Rekam kecepatan dan percepatan puncak di detik jatuhnya bola
        ...
        // 2. Rekam angka 0 di detik yang SAMA agar terbentuk garis lurus (90 derajat)
...
function drawBall(ball, color) {
    const displayRadius = Math.max(12, Math.min(35, ball.diameter * 60));
    ctx.fillStyle = 'rgba(0, 0, 0, 0.3)'; ctx.beginPath(); ctx.ellipse(ball.x, groundY + 5, displayRadius * 0.8, displayRadius * 0.3, 0, 0, Math.PI * 2); ctx.fill();
    const ballGradient = ctx.createRadialGradient(ball.x - displayRadius * 0.3, ball.y - displayRadius * 0.3, 0, ball.x, ball.y, displayRadius);
    ballGradient.addColorStop(0, color); ballGradient.addColorStop(1, '#000'); 
    ctx.fillStyle = ballGradient; ctx.beginPath(); ctx.arc(ball.x, ball.y, displayRadius, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = 'rgba(255, 255, 255, 0.4)'; ctx.beginPath(); ctx.arc(ball.x - displayRadius * 0.3, ball.y - displayRadius * 0.3, displayRadius * 0.25, 0, Math.PI * 2); ctx.fill();
    if (ball.velocity > 5 && ball.isActive) {
        ctx.globalAlpha = 0.3; ctx.fillStyle = color;
        for (let i = 1; i <= 3; i++) { ctx.beginPath(); ctx.arc(ball.x, ball.y - i * (ball.velocity * 0.2), displayRadius, 0, Math.PI * 2); ctx.fill(); }
        ctx.globalAlpha = 1;
    }
}
...
heightLeftRange.addEventListener('input', function(e) {
    heightLeftInput.value = e.target.value; 
    if (leftBall && !isRunning) updateBallPosition('left');
});

heightLeftInput.addEventListener('input', function(e) {
    let val = parseFloat(e.target.value);
    if (isNaN(val)) val = 0;
    if (val > 75) val = 75; 
    heightLeftRange.value = val; 
    if (leftBall && !isRunning) updateBallPosition('left');
});
...
heightRightRange.addEventListener('input', function(e) {
    heightRightInput.value = e.target.value; 
    if (rightBall && !isRunning) updateBallPosition('right');
});

heightRightInput.addEventListener('input', function(e) {
    let val = parseFloat(e.target.value);
    if (isNaN(val)) val = 0;
    if (val > 75) val = 75;
    heightRightRange.value = val; 
    if (rightBall && !isRunning) updateBallPosition('right');
});
...
// Start
initStars();
initializeBalls();.destroy(); if (chart2) chart2.destroy();
    Chart.defaults.color = '#ccc'; Chart.defaults.borderColor = 'rgba(255, 255, 255, 0.1)'; 
    updateChartConfig(1); updateChartConfig(2);
}
        ...
        data: {
            datasets: [
                { 
                    label: 'Kanan (Biru)', data: [], borderColor: '#00d2ff', backgroundColor: 'rgba(0, 210, 255, 0.1)', 
                    borderWidth: 2, pointRadius: 0, 
                    tension: 0 // Hapus cubicInterpolationMode agar garis tegak lurus sempurna
                },
                { 
                    label: 'Kiri (Merah)', data: [], borderColor: '#ff4757', backgroundColor: 'rgba(255, 71, 87, 0.1)', 
                    borderWidth: 2, pointRadius: 0, 
